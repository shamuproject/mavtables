...
TEST_CASE("IPAddress's are copyable.", "[IPAddress]")
{
    ...
    IPAddress address_a_copy = address_a;
    IPAddress address_b_copy(address_b);
          ...
          "[IPAddress]")
{
    IPAddress address_a(0x00000000, 0);
    IPAddress address_b(0xFFFFFFFF, 65535);
    IPAddress address_a_copy(address_a, 65535);
    IPAddress address_b_copy(address_b, 0);
    REQUIRE(&address_a != &address_a_copy);
    REQUIRE(address_a.address() == address_a_copy.address());
    REQUIRE(address_a.port() == 0);
    REQUIRE(address_a_copy.port() == 65535);
    REQUIRE(&address_b != &address_b_copy);
    REQUIRE(address_b.address() == address_b_copy.address());
    REQUIRE(address_b.port() == 65535);
    REQUIRE(address_b_copy.port() == 0);
    SECTION("And ensures port number is within range.")
    {
        REQUIRE_THROWS_AS(
            IPAddress(address_a, static_cast<unsigned int>(-1)),
            std::out_of_range);
        REQUIRE_THROWS_AS(
            IPAddress(address_b, static_cast<unsigned int>(-1)),
            std::out_of_range);
        REQUIRE_THROWS_AS(IPAddress(address_a, 65536), std::out_of_range);
        REQUIRE_THROWS_AS(IPAddress(address_b, 65536), std::out_of_range);
    }
}
...
TEST_CASE("IPAddress's are movable.", "[IPAddress]")
{
    IPAddress address_a(0x00000000, 0);
    IPAddress address_b(0xFFFFFFFF, 65535);
    IPAddress address_a_moved = std::move(address_a);
    IPAddress address_b_moved(std::move(address_b));
    REQUIRE(address_a_moved == IPAddress(0x00000000, 0));
    REQUIRE(address_b_moved == IPAddress(0xFFFFFFFF, 65535));
}
...
TEST_CASE("IPAddress's are assignable.", "[IPAddress]")
{
    IPAddress address_a(0x00000000, 0);
    IPAddress address_b(0xFFFFFFFF, 65535);
    REQUIRE(address_a == IPAddress(0x00000000, 0));
    address_a = address_b;
    REQUIRE(address_a == IPAddress(0xFFFFFFFF, 65535));
}
