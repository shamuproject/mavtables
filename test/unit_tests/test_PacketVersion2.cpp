...
TEST_CASE("packet_v2::Packet's can be constructed.", "[packet_v2::Packet]")
{
    ...
    SECTION("And ensures the message ID is valid.")
    {
        ...
        REQUIRE_THROWS_AS(
            packet_v2::Packet(to_vector(ping)), InvalidPacketIDError);
        REQUIRE_THROWS_WITH(
            packet_v2::Packet(to_vector(ping)),
        ...
        REQUIRE_THROWS_WITH(
            packet_v2::Packet(to_vector(ping)),
            "Packet ID (#5000) is not part of the '"
            MAVLINK_DIALECT "' MAVLink dialect.");
    }
...
TEST_CASE("packet_v2::Packet's are assignable.", "[Packet]")
{
    ...
    REQUIRE(packet_a == packet_v2::Packet(to_vector(PingV2())));
    packet_a = packet_b;
    REQUIRE(packet_b == packet_v2::Packet(to_vector(SetModeV2())));
}
