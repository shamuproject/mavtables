...
TEST_CASE(
    "UnixUDPSocket's create and bind a UDP socket on construction and "
    ...
    "[UnixUDPSocket]")
{
    SECTION("Without a specific IP address (no errors).")
    {
        ...
        fakeit::When(Method(mock_sys, bind))
            .Do([&](auto fd, auto addr, auto addrlen) {
                (void)fd;
        ...
        fakeit::When(Method(mock_sys, close)).Return(0);
        {
            ...
            fakeit::Verify(
                Method(mock_sys, socket)
                    .Matching([&](auto family, auto type, auto protocol) {
                        return family == AF_INET && type == SOCK_DGRAM &&
                               protocol == 0;
                    }))
                .Once();
            fakeit::Verify(Method(mock_sys, bind)
                               .Matching([&](auto fd, auto addr, auto addrlen) {
                                   (void)addr;
    ...
    SECTION("Emmits errors from 'socket' system call.")
    {
        ...
        std::array<int, 7> errors{{EACCES, EAFNOSUPPORT, EINVAL, EMFILE,
    ...
    SECTION("Emmits errors from 'bind' system call.")
    {
        ...
        std::array<int, 13> errors{{EACCES, EADDRINUSE, EBADF, EINVAL, ENOTSOCK,
                                    EADDRNOTAVAIL, EFAULT, ELOOP, ENAMETOOLONG,
    ...
    "[UnixUDPSocket]")
{
    ...
    SECTION("Without error.")
    {
        ...
        struct sockaddr_in address;
        fakeit::When(Method(mock_sys, sendto))
            .Do([&](auto fd, auto buf, auto len, auto flags, auto addr,
                    auto addrlen) {
                (void)fd;
                (void)flags;
                sent.resize(len);
        ...
        // Verify 'sendto'.
        fakeit::Verify(Method(mock_sys, sendto)
                           .Matching([](auto fd, auto buf, auto len, auto flags,
    ...
    SECTION("Emmits errors from 'sendto' system call.")
    {
        ...
        std::array<int, 18> errors{
            {EACCES, EAGAIN, EWOULDBLOCK, EALREADY, EBADF, ECONNRESET,
    ...
    "[UnixUDPSocket]")
{
    ...
    SECTION("Packet available (no errors).")
    {
        ...
        // Mock 'ioctl'.
        fakeit::When(Method(mock_sys, ioctl))
            .Do([](auto fd, auto request, auto size) {
                (void)fd;
                (void)request;
        ...
        fakeit::When(Method(mock_sys, recvfrom))
            .Do([&](auto fd, auto buf, auto len, auto flags, auto addr,
                    auto addrlen) {
                (void)fd;
                (void)flags;
                ...
                address.sin_addr.s_addr =
                    htonl(static_cast<uint32_t>(1234567890));
        ...
        REQUIRE(fds.revents == 0);
        // Verify 'ioctl'.
        fakeit::Verify(Method(mock_sys, ioctl)
                           .Matching([](auto fd, auto request, auto size) {
                               (void)size;
                               return fd == 3 && request == FIONREAD;
                           }))
            .Once();
        // Verify 'recvfrom'.
        fakeit::Verify(Method(mock_sys, recvfrom)
                           .Matching([](auto fd, auto buf, auto len, auto flags,
                                        auto addr, auto addrlen) {
                               (void)buf;
                               (void)addr;
    ...
    SECTION("Packet available (IP address truncated).")
    {
        ...
        fakeit::When(Method(mock_sys, recvfrom))
            .Do([&](auto fd, auto buf, auto len, auto flags, auto addr,
                ...
                address_length = *addrlen;
                *addrlen =
                    static_cast<socklen_t>(std::min(
                        static_cast<size_t>(*addrlen), sizeof(address))) +
                    1;
                // Return number of received bytes.
                return std::min(vec.size(), len);
            });
    ...
    SECTION("Emmits errors from 'recvfrom' system call.")
    {
        ...
        // Mock ioctl system call.
        fakeit::When(Method(mock_sys, ioctl))
            .AlwaysDo([](auto fd, auto request, auto size) {
                (void)fd;
                (void)request;
                *reinterpret_cast<int *>(size) = 4;
                return 0;
        ...
        std::array<int, 13> errors{
            {EACCES, EWOULDBLOCK, EBADF, ECONNRESET, EINTR, EINVAL, ENOTCONN,
    ...
    SECTION("Emmits errors from 'ioctl' system call.")
    {
        ...
        // Test
        std::array<int, 4> errors{{EBADF, EFAULT, EINVAL, ENOTTY}};
