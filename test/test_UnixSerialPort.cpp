...
TEST_CASE(
    "UnixSerialPort's open and configure a serial port on construction"
    "and closes the port on destruction.",
    "[UnixSerialPort]")
{
    SECTION("Without hardware flow control (no errors).")
    {
        ...
        // Mock 'tcgetattr'.
        fakeit::When(Method(mock_sys, tcgetattr))
            .AlwaysDo([&](auto fd, auto termios_p) {
                (void)fd;
        ...
        // Mock 'tcsetattr'.
        struct termios tty;
        fakeit::When(Method(mock_sys, tcsetattr))
            .AlwaysDo([&](auto fd, auto action, auto termios_p) {
                (void)fd;
                (void)action;
        ...
        fakeit::When(Method(mock_sys, close)).AlwaysReturn(0);
        {
            ...
            fakeit::Verify(
                Method(mock_sys, open).Matching([](auto path, auto flags) {
                    return std::string(path) == "/dev/ttyUSB0" &&
                           flags == (O_RDWR | O_NOCTTY | O_SYNC);
                }))
                .Once();
            // Verify 'tcgetattr'.
            fakeit::Verify(Method(mock_sys, tcgetattr)
                               .Matching([&](auto fd, auto termios_p) {
                                   (void)termios_p;
                                   return fd == 3;
                               }))
                .Once();
            // Verify 'tcsetattr'.
            fakeit::Verify(
                Method(mock_sys, tcsetattr)
                    .Matching([](auto fd, auto action, auto termios_p) {
                        (void)termios_p;
    ...
    SECTION("With hardware flow control (no errors).")
    {
        ...
        struct termios tty;
        fakeit::When(Method(mock_sys, tcsetattr))
            .Do([&](auto fd, auto action, auto termios_p) {
                (void)fd;
                (void)action;
                std::memcpy(&tty, termios_p, sizeof(struct termios));
                return 0;
        ...
        fakeit::When(Method(mock_sys, close)).Return(0);
        {
            // Construct port.
            UnixSerialPort port(
                "/dev/ttyUSB0", 9600, SerialPort::FLOW_CONTROL,
                mock_unique(mock_sys));
            // Verify 'open' system call.
            fakeit::Verify(
                Method(mock_sys, open).Matching([](auto path, auto flags) {
    ...
    SECTION("Emmits errors from 'open' system call.")
    {
        ...
        std::array<int, 26> errors{
            {EACCES,     EDQUOT,    EEXIST, EFAULT, EFBIG,        EINTR,
             EINVAL,     EISDIR,    ELOOP,  EMFILE, ENAMETOOLONG, ENFILE,
             ENODEV,     ENOENT,    ENOMEM, ENOSPC, ENOTDIR,      ENXIO,
    ...
    SECTION("Emmits errors from 'tcgetattr' system call, and closes the port.")
    {
        ...
        std::array<int, 2> errors{{EBADF, ENOTTY}};
    ...
    SECTION("Emmits errors from 'tcsetattr' system call, and closes the port.")
    {
        ...
        std::array<int, 5> errors{{EBADF, EINTR, EINVAL, ENOTTY, EIO}};
    ...
    "[UnixSerialPort]")
{
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 0, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 50, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 75, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 110, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 134, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 135, SerialPort::DEFAULT, mock_unique(mock_sys));
        REQUIRE(cfgetispeed(&tty) == B134);
        REQUIRE(cfgetospeed(&tty) == B134);
    }
    SECTION("150 bps")
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 150, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 200, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 300, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 600, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 1200, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 1800, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 2400, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 4800, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 9600, SerialPort::DEFAULT, mock_unique(mock_sys));
        REQUIRE(cfgetispeed(&tty) == B9600);
        REQUIRE(cfgetospeed(&tty) == B9600);
    }
    SECTION("19200 bps")
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 19200, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 38400, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 57600, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 115200, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    {
        UnixSerialPort port(
            "/dev/ttyUSB0", 230400, SerialPort::DEFAULT, mock_unique(mock_sys));
    ...
    "[UnixSerialPort]")
{
    ...
    SECTION("Timeout, no data (no errors).")
    {
        // Mock 'poll'.
        struct pollfd fds;
        fakeit::When(Method(mock_sys, poll))
            .Do([&](auto fds_, auto nfds, auto timeout) {
                (void)timeout;
        ...
        // Verify 'poll'.
        fakeit::Verify(Method(mock_sys, poll)
                           .Matching([](auto fds_, auto nfds, auto timeout) {
                               (void)fds_;
    ...
    SECTION("Poll error, close and reopen the serial port (no other errors).")
    {
        ...
        struct pollfd fds;
        fakeit::When(Method(mock_sys, poll))
            .AlwaysDo([&](auto fds_, auto nfds, auto timeout) {
                (void)timeout;
                std::memcpy(&fds, fds_, nfds * sizeof(fds));
        ...
        // Verify 'tcgetattr'.
        fakeit::Verify(
            Method(mock_sys, tcgetattr).Matching([&](auto fd, auto termios_p) {
                (void)termios_p;
                return fd == 3;
            }))
            .Exactly(2);
        // Verify 'tcsetattr'.
        fakeit::Verify(Method(mock_sys, tcsetattr)
                           .Matching([](auto fd, auto action, auto termios_p) {
                               (void)termios_p;
                               return fd == 3 && action == TCSANOW;
                           }))
    ...
    SECTION("Data available (no errors).")
    {
        ...
        // Mock 'read'.
        fakeit::When(Method(mock_sys, read))
            .Do([](auto fd, auto buf, auto count) {
                (void)fd;
                // Write to buffer.
        ...
        // Verify 'read'.
        fakeit::Verify(
            Method(mock_sys, read).Matching([](auto fd, auto buf, auto count) {
                (void)buf;
    ...
    SECTION("Emmits errors from 'poll' system call.")
    {
        ...
        // Test
        std::array<int, 4> errors{{EFAULT, EINTR, EINVAL, ENOMEM}};
    ...
    "[UnixSerialPort]")
{
    ...
    SECTION("Without error.")
    {
        ...
        fakeit::When(Method(mock_sys, write))
            .Do([&](auto fd, auto buf, auto count) {
                (void)fd;
        ...
        // Verify 'write'.
        fakeit::Verify(
            Method(mock_sys, write).Matching([](auto fd, auto buf, auto count) {
                (void)buf;
                return fd == 3 && count == 4;
            }))
            .Once();
    ...
    SECTION("Emmits errors from 'write' system call.")
    {
        ...
        std::array<int, 11> errors{{EAGAIN, EBADF, EDESTADDRREQ, EDQUOT, EFAULT,
                                    EFBIG, EINTR, EINVAL, EIO, ENOSPC, EPIPE}};
