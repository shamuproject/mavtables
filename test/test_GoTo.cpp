...
TEST_CASE(
    "GoTo's 'action' method determines what to do with a "
    ...
    "[GoTo]")
{
    ...
    {
        REQUIRE(
            GoTo(std::make_shared<TestChain>())
    ...
    SECTION("Delegates to the contained chain if the conditional is a match.")
    {
        // Without priority.
        REQUIRE(
            GoTo(accept_chain, If().to("192.168"))
                .action(ping, MAVAddress("192.168")) == Action::make_accept());
        REQUIRE(
            GoTo(reject_chain, If().to("192.168"))
                .action(ping, MAVAddress("192.168")) == Action::make_reject());
        REQUIRE(
            GoTo(continue_chain, If().to("192.168"))
                .action(ping, MAVAddress("192.168")) == Action::make_default());
        REQUIRE(
            GoTo(default_chain, If().to("192.168"))
                .action(ping, MAVAddress("192.168")) == Action::make_default());
        // With priority (adds priority).
        REQUIRE(
            GoTo(accept_chain, 3, If().to("192.168"))
                .action(ping, MAVAddress("192.168")) == Action::make_accept(3));
        // Priority already set (no override).
        REQUIRE(
            GoTo(accept10_chain, 3, If().to("192.168"))
    ...
    SECTION("Returns the continue action if the conditional does not match.")
    {
        // Without priority.
        REQUIRE(
            GoTo(accept_chain, If().to("172.16"))
                .action(ping, MAVAddress("192.168")) ==
            Action::make_continue());
        // With priority.
        REQUIRE(
            GoTo(accept_chain, 3, If().to("172.16"))
                .action(ping, MAVAddress("192.168")) ==
            Action::make_continue());
    }
}
...
TEST_CASE("GoTo's are printable (without a condition or a priority).", "[GoTo]")
{
    ...
    GoTo goto_(chain);
    Rule &rule = goto_;
    SECTION("By direct type.") { REQUIRE(str(goto_) == "goto test_chain"); }
    SECTION("By polymorphic type.") { REQUIRE(str(rule) == "goto test_chain"); }
}


TEST_CASE(
    "GoTo's are printable (without a condition but with a priority).", "[GoTo]")
{
    auto chain = std::make_shared<TestChain>();
    auto ping = packet_v2::Packet(to_vector(PingV2()));
    GoTo goto_(chain, -3);
...
TEST_CASE(
    "GoTo's are printable (with a condition but without a priority).", "[GoTo]")
{
    auto chain = std::make_shared<TestChain>();
    auto ping = packet_v2::Packet(to_vector(PingV2()));
